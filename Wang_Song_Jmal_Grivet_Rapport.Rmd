...
On voit sur la figure \ref{fig:intraclasse}, pour l'inertie intraclasse, qu'il y a un coude pour K = 3 ou K = 6. On retient donc 3 classes par ce critère. Le critère silhouette montre un pic à K = 2, donc on retient 2 classes avec ce critère.
\vspace{1em}
...
On compare les deux classifications: k = 2 avec K-means et k=3 avec CAH Ward:
...
On voit sur la figure \ref{fig:chord} que le cluster 1 de k-means est égale aux cluster 2 et 3 de CAH, qui représente 2h et 3h du traitement T2/T3 du réplicat R1/R2. Les gènes du traitements T2/T3 dans les heures aux milieux(2h, 3h) ont donc des comportements différents que l'heure débutante(1h) et heures finales(5h,6h), ce qui correspond à l'analyse précédente.
...
On garde toutes les variables mais on observe quand même une gradation. Le temps précédent (5h) est le plus influent suivi du temps de démarrage (1h, 2h). On peut faire l'hypothèse d'une périodicité de temps sur l'influence des traitements sur les gènes. Il faudrait tester cette sélection de variable sur plus d'heures afin de valider ou non cette hypothèse.
...
```{r, echo = F, fig.height=3, fig.cap="\\label{fig:lasso}Sélection de variable avec le critère lasso"}
tildeX = as.matrix(R2[,-18])
tildeY = data$T3_6h_R2
...
Peut importe les combinaisons de traitement qu'on prend en pour expliquer T3_6h_R2, on obtient la même erreur (des p-valeurs toutes égales à 1) sauf lorsqu'on prend seulement le traitement 1. \newline Dans ce cas, on obtient que T3_6h_R2 s'explique par T1_1h_R2, T1_2h_R2, T1_3h_R2, T1_4h_R2, T1_6h_R2 par une sélection de variable "backward". Autrement dit, seulement on enlève seulement l'heure 5 ce qui ne nous aide pas vraiment. \newline
Toutes les sélections de variable et les combinaisons citée ici sont dans le Rmd.
...
```{r, echo = F, fig.height=3, fig.cap="\\label{fig:lasso2}Sélection de variable avec le critère lasso"}
lambda_seq=seq(0,1,0.001)
x = model.matrix(T1_6h_R2~.,data=R2)
y = data$T1_6h_R2
