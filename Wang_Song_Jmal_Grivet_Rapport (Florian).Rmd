...
data_centree_reduite = as.data.frame(data_centree_reduite)
res.pca = PCA(data_centree_reduite, ncp=15, graph=F)
summary(res.pca)
```
...
```{r include=FALSE}
data_transpose = t(data_centree_reduite)
...
Cependant, lorsqu'on fait une sélection de variable "backward" sur notre modèle, on obtient que T3_6h_R2 peut s'expliquer par les variables : T1_4h_R2, T1_6h_R2, T2_5h_R2, T3_3h_R2.
...
```{r include=F}
stepAIC(glm.T1R2_T3_6h_R2, direction=c("forward"),p=2, trace=F) # AIC
```

```{r include=F}
# D'autres combinaisons
# Seulement T2
T2 = data[grep("T2", names(data), value=TRUE)]
T2R2 = T2[grep("R2", names(T2), value=TRUE)]
T2R2["T3_6h_R2"] = data$T3_6h_R2
T36HR2_binomial = T2R2[-c(6)]
T36HR2_binomial$T3_6h_R2[T36HR2_binomial$T3_6h_R2>1] = 1
T36HR2_binomial$T3_6h_R2[T36HR2_binomial$T3_6h_R2<(-1)] = 0
T36HR2_binomial$T3_6h_R2 = as.factor(T36HR2_binomial$T3_6h_R2)
glm.T36HR2<-glm(T3_6h_R2~.,data=T36HR2_binomial,family=binomial(link="logit"), control = glm.control(maxit = 100))
summary(glm.T36HR2)
step.backward <- step(glm.T36HR2, trace=F)
summary(step.backward)

# T2 et T3
T36HR2_binomial = cbind(T3R2, T2R2)
T36HR2_binomial$T3_6h_R2[T36HR2_binomial$T3_6h_R2>1] = 1
T36HR2_binomial$T3_6h_R2[T36HR2_binomial$T3_6h_R2<(-1)] = 0
T36HR2_binomial = T36HR2_binomial[-c(12, 13)] # T3R2 et T2R2 sauf le temps 6
T36HR2_binomial$T3_6h_R2 = as.factor(T36HR2_binomial$T3_6h_R2)
glm.T36HR2<-glm(T3_6h_R2~.,data=T36HR2_binomial,family=binomial(link="logit"), control = glm.control(maxit = 100))
summary(glm.T36HR2)
step.backward <- step(glm.T36HR2, trace=F)
summary(step.backward)
stepAIC(glm.T36HR2, direction=c("forward"),p=2, trace=0) # AIC
```

# Fin A AJOUTER FLORIAN 1
...
```{r}
modelnonord <-vglm(Y ~ ., data = dfmodel, family = multinomial)
summary(modelnonord)
```

## A Analyser ou à enlever si on ne sait pas quoi dire

# FIN A AJOUTER FLORIAN 2
...
lambda_seq=seq(0,1,0.001)
x = model.matrix(T3_6h_R2~.,data=R2)
y = T36HR2_binomial$T3_6h_R2
fitlasso <- glmnet(x, y , alpha = 1, lambda = lambda_seq, family=c("binomial"), intercept=F)
plot(fitlasso,label= TRUE)
abline(v = 0.7)
coefs <- coef(fitlasso, s="lasso")
plot_ly(data = coefs, x = ~lambda, y = ~coef, type = 'scatter', mode = 'lines') %>%
  layout(title = "Lasso Regression",
...
```{r}
df = data.frame(
  lambda = rep(fitlasso$lambda, ncol(x)),
  theta = as.vector(t(fitlasso$beta)),
  variables = rep(colnames(x), each=length(fitlasso$lambda))
)
