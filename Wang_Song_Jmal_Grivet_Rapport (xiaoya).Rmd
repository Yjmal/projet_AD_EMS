...
On choisit le nombre de classe en utilisant le critère silhouette:
```{r, eval=F}
library(cluster)
Silhou<-NULL
for (k in 2:Kmax){
...
```
on retirnt 2 classes selon le graphe, on représente sur les 2 axes de l'ACP:

```{r, fig.height=3}
km = kmeans(coord,centers = 2)
fviz_cluster(km,data=coord,axes = c(1,2), ellipse.type="norm",labelsize=8,geom=c("point","text"))+ggtitle("")
```
Cluster 1 regroupe traitement T2/T3 du répicat R1/R2 de 2h à 6h, cluster 2 regroupe traitement T2/T3 du répicat R1/R2 à 1h et traitement T1 du réplicat R1/R2 de 1h à 6h.
...
Les partitions des deux clusters sont plutôt homogènes:
```{r, fig.height=3}
...
Les 4 classifications hiérarchiques:
```{r, fig.height=3}
d = dist(coord)
...
```
Selon l'hauteur du dendogramme, on voitqu'il y a une saute en k = 3 ou 6 et puis la courbe s'aplanit, on retient donc 3 ou 6 classes.

On détermine le nombre de classes à retenir avec l’indice de Calinski_Harabasz: ne marche pas ?
```{r, fig.height=3}
library(clusterSim)
...
```
On voit que plus k est grand, plus indice Calinski-Harabasz augemente, il n'y apas de pic sur la courbe, donc on peut pas déterminer une classification avec le critère Calinski-Harabasz.

On essaie le critère silhouette:
```{r, fig.height=3}
Silhou<-NULL
Kmax<-20
for (k in 2:Kmax){
...
```
2 classes avec silhouette

On trace la distribution des variables en fonction de la classification en 3 classes avec la mésure d'agrégation Ward: 
## à commenter
```{r, fig.height=3}
library(reshape)
...
On travaille avec le data non transposé pour la calassification des gènes:
